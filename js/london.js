...
<div class="info-data">to see flow details</div>`);
    ...
    const { locationId, name, totalInflow, totalOutflow, connectionData, flowConnections } = data;
            ...
            <div class="info-data">
                <div class="flow-summary">
                    ...
                    <div class="net-flow">
                        ...
                        <span class="value ${totalInflow - totalOutflow >= 0 ? 'positive' : 'negative'}">
                            ${totalInflow - totalOutflow >= 0 ? '+' : ''}${formatNumber(totalInflow - totalOutflow)}
                        </span>
                    </div>
                </div>
            </div>`);
...
function createVisualization(lsoas_21, flowsMap, namesMap, areaFlowTotals, lsoa_connections, flows, msoa_21) {
    const width = 1260;
    const height = 900;
...
async function loadData() {
    ...
    // 检查 LSOA 数据中是否包含 MSOA 编码
    console.log("Sample LSOA properties:", lsoas_21[0].properties);
    ...
    flows.forEach(flow => {
        ...
        // 更新总流量
        areaFlowTotals.set(sourceId, (areaFlowTotals.get(sourceId) || 0) + flowSize);
...
async function init() {
    ...
    const namesMap = new Map(
        lsoas_21.map(d => [
            d.properties.lsoa21cd,
            d.properties.LSOA21NM
        ])
    );

    // 修改可视化函数调用，添 flows 参数
    createVisualization(lsoas_21, flowsMap, namesMap, areaFlowTotals, lsoa_connections, flows, msoa_21);
